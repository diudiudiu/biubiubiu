...
会影响子元素的 position：fixed的定位 ，其子元素的相对的不再是页面的左上角，而是父元素的左上角，fixed元素不会脱离文档流，但是 top 等属性依然可用。



当给一个元素加上transform属性的时候，这个元素就会具有relative的特性，有着 z-index 相对定位的特性，但是left top right bottom 效果不起作用，若一个元素的父元素拥有tranform属性，那么子元素在使用定位属性的时候要注意。
